...
8. Milestones or Phases
   - Break work into verifiable stages. Each stage must include acceptance criteria.
   - 把工作拆成可验证阶段。每个阶段必须有验收标准。

9. Work Loop for Agents
   - Tell agents how to resume, choose work, implement, verify, log progress, and continue.
   - 告诉 agent 如何恢复上下文、选择任务、实现、验证、记录进度并继续。
...
4. Run quick validation to establish baseline.
5. Select the highest-priority unfinished task.
6. Implement a small verifiable increment.
7. Run relevant validation.
8. Fix failures introduced by the increment.
9. Update worklog, decisions, and validation records.
10. Continue if budget remains and no blocking condition exists.
```
