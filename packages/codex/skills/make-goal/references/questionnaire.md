...
5. What execution harness does the agent need?
   - OS, shell, language runtimes, package managers, browsers, CLIs, MCP servers, plugins, local apps, cloud services, databases.
   - agent 需要什么执行环境？操作系统、shell、语言运行时、包管理器、浏览器、CLI、MCP、插件、本地应用、云服务、数据库等。

6. What setup and permissions are required?
   - Install commands, env vars, credentials, network access, filesystem scope, approval rules, deployment/publish permissions.
   - 需要哪些安装命令、环境变量、凭据、网络访问、文件范围、审批规则、部署/发布权限？

7. What should agents do if harness pieces are missing?
   - Bootstrap automatically, use mocks, skip optional work, continue offline, ask the user, or stop.
   - 如果环境能力缺失，agent 应该自动安装、用 mock、跳过可选项、离线继续、询问用户，还是停止？

8. What constraints matter?
...
- Should the goal include a local-only fallback when cloud or paid services are unavailable?
- What commands prove the harness is ready before real work starts?
...
- If the agent will execute in an unknown environment, include a harness discovery phase.
